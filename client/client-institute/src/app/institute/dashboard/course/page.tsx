...
function InstituteCourse() {
  const { courses: instituteCourses } = useAppSelector((store) => store.course);
  const [searchingText, setSearchingText] = useState<string>("");
  const dispatch = useAppDispatch();
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const openModal = () => setIsModalOpen(true);
  const closeModal = () => setIsModalOpen(false);
  const filteringSearchingData = instituteCourses.filter(
    (course) =>
      course.id.includes(searchingText) ||
      course.courseName.toLowerCase().includes(searchingText.toLowerCase()) ||
      course.courseDescription
        .toLowerCase()
        .includes(searchingText.toLowerCase())
  );
  const handleCourseDelete = async (cId: string): Promise<void | ""> =>
    cId && (await dispatch(deleteInstituteCourse(cId)));
  useEffect(() => {
      ...
      <div className=" overflow-x-auto">
        ...
        <div className="min-w-full inline-block align-middle">
          <div className="flex justify-between">
            ...
            <button
              onClick={openModal}
              className="bg-green-400 h-[34px] p-2 rounded pb-1"
            >
              + Course
            </button>
          </div>
        ...
        <div className="overflow-hidden ">
          <table className=" min-w-full rounded-xl">
            ...
            <tbody className="divide-y divide-gray-300 ">
              {/* Mapping It */}
              {filteringSearchingData.length > 0 &&
                filteringSearchingData.map((icourse) => {
                  return (
                    <tr
                      key={`${icourse?.id}-${icourse?.courseDescription}`}
                    ...
                    >
                      ...
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                        {icourse.courseDescription}
                      </td>
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                        {new Date(
                          icourse?.createdAt?.toString()
                        ).toLocaleDateString()}
                      </td>
                      <td className=" p-5 ">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            onClick={() => handleCourseDelete(icourse?.id)}
                            className="p-2 rounded-full cursor-pointer  group transition-all duration-500  flex item-center"
                          >
                            <svg
                              width={20}
                              height={20}
